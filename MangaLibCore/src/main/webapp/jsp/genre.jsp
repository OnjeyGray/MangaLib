            ...
            <custom:checkAcces currentRole="${authorizedUser.role}" accessRole="ADMIN">
                <div class="card mt-4">
                    ...
                    <div class="card-body">
                        ...
                        <form action="/controller/editGenre?attribute=genreName&genreId=${genre.id}" method="post" class="row">
                        ...
                        <form action="/controller/editGenre?attribute=genreDescription&genreId=${genre.id}" method="post" class="row">
                            <div class="form-group input-group col-8">
                                <textarea required pattern="^[^<>]{1,1000}$" name="genreDescription"
                                          placeholder="<fmt:message key="key.description" bundle="${lang}"/>"
                                          class="form-control">${genre.description}</textarea>
                            </div>
                            <div class="col-4">
                                <input type="submit" class="btn btn-primary btn-block"
                                       value="<fmt:message key="key.save" bundle="${lang}"/>">
                        ...
                        <div class="mt-4">
                            <a href="/controller/editGenre?attribute=genre&action=delete&genreId=${genre.id}" class="btn btn-danger btn-block">
